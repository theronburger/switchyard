...
struct OperationWaitTests {
    ...
    private static func snapshot(prepareState: String) throws -> StatusSnapshot {
        var root = try #require(JSONSerialization.jsonObject(with: Data(contentsOf: fixtureURL)) as? [String: Any])
        ...
        operations.append([
            "id": "operation_prepare_wait", "kind": "prepareWorktree", "state": prepareState,
            "createdAt": "2026-08-14T09:56:00Z", "updatedAt": "2026-08-14T09:56:30Z",
        ])
        root["operations"] = operations
        return try ContractDecoder().decode(StatusSnapshot.self, from: JSONSerialization.data(withJSONObject: root))
    }

    @Test
    func `waiting for an operation reads status only and never re-enters the lifecycle controller`() async throws {
        let pending = try Self.snapshot(prepareState: "running")
        let succeeded = try Self.snapshot(prepareState: "succeeded")
        let session = DaemonSession(
            instanceId: pending.daemon.instanceId, daemonVersion: pending.daemon.version,
            endpoint: EndpointDescriptor(
                schemaVersion: contractSchemaVersion, transport: "http", host: "127.0.0.1", port: 49402,
                daemonVersion: pending.daemon.version, instanceId: pending.daemon.instanceId,
                pid: 4242, createdAt: pending.daemon.startedAt
            )
        )
        let report = DoctorReport(checks: [DoctorCheck(id: "live", title: "Live", outcome: .passed("healthy"))])
        let lifecycle = CountingLifecycleController(
            result: DaemonLifecycleResult(state: .ready(session), snapshot: pending, doctorReport: report)
        )
        // Three pending reads, one transient failure, then success.
        let statusReads = ScriptedStatusProvider(results: [
            .success(pending), .success(pending), .success(pending),
            .failure(DaemonClientError.transportFailure("simulated status failure")),
            .success(succeeded),
        ])
        let receipt = try ContractDecoder().decode(MutationReceipt.self, from: Data("""
        {"schemaVersion":2,"requestId":"app_wait","operationId":"operation_prepare_wait","acceptedAt":"2026-08-14T09:56:00Z"}
        """.utf8))
        let workspaceActions = StubWorkspaceActions(receipt: receipt)
        let model = AppModel(
            liveController: lifecycle,
            workspaceActions: workspaceActions,
            configurationActions: nil,
            agentConnections: nil,
            operationStatus: statusReads,
            pollingInterval: .seconds(60),
            operationPollingInterval: .milliseconds(5)
        )
        await model.refresh()
        let refreshesBefore = await lifecycle.refreshCount
        #expect(refreshesBefore == 1)
        ...
        let refreshesAfter = await lifecycle.refreshCount
        let repairs = await lifecycle.repairCount
        let reads = await statusReads.reads
        #expect(refreshesAfter == 1)
        #expect(repairs == 0)
        #expect(reads == 5)
        let waited = model.snapshot?.operations.first(where: { $0.id == "operation_prepare_wait" })
        #expect(waited?.state == .succeeded)
        // The transient status failure never disturbed lifecycle state.
        #expect(model.lifecycleState == .ready(session))
    }
}

private actor CountingLifecycleController: DaemonLifecycleControlling {
    let result: DaemonLifecycleResult
    private(set) var refreshCount = 0
    private(set) var repairCount = 0

    init(result: DaemonLifecycleResult) { self.result = result }
...
private actor ScriptedStatusProvider: StatusProviding {
    private var results: [Result<StatusSnapshot, Error>]
    private(set) var reads = 0

    init(results: [Result<StatusSnapshot, Error>]) { self.results = results }

    nonisolated var sourceDescription: String { "scripted" }

    func loadStatus() async throws -> StatusSnapshot {
        reads += 1
        guard !results.isEmpty else { throw DaemonClientError.transportFailure("script exhausted") }
        let next = results.count > 1 ? results.removeFirst() : results[0]
        return try next.get()
    }
}

private struct StubWorkspaceActions: WorkspaceActionSubmitting {
    ...
    func prepareWorktree(_ request: PrepareWorktreeRequest) async throws -> MutationReceipt { receipt }
}
