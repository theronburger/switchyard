...
type actionStubBackend struct {
	stubBackend
	list func(context.Context) (contractv1.ProfileActionList, error)
	run  func(context.Context, contractv1.RunProfileActionRequest) (contractv1.MutationReceipt, error)
}

func (b actionStubBackend) ListProfileActions(ctx context.Context) (contractv1.ProfileActionList, error) {
	if b.list == nil {
		return contractv1.ProfileActionList{}, errors.New("list is not configured")
	}
	return b.list(ctx)
}

func (b actionStubBackend) RunProfileAction(ctx context.Context, request contractv1.RunProfileActionRequest) (contractv1.MutationReceipt, error) {
	if b.run == nil {
		return contractv1.MutationReceipt{}, errors.New("run is not configured")
	}
	return b.run(ctx, request)
}

func TestApplicationActionsListsAcceptedActions(t *testing.T) {
	backend := actionStubBackend{list: func(context.Context) (contractv1.ProfileActionList, error) {
		return contractv1.ProfileActionList{
			SchemaVersion: contractv1.SchemaVersion, Actions: []contractv1.ProfileAction{{
...
func TestApplicationActionBuildsScopedRequest(t *testing.T) {
	...
	backend := actionStubBackend{
		...
		run: func(_ context.Context, request contractv1.RunProfileActionRequest) (contractv1.MutationReceipt, error) {
...
func TestApplicationActionDotResolvesCurrentWorktreeAndWaits(t *testing.T) {
	...
	var submitted contractv1.RunProfileActionRequest
	backend := actionStubBackend{
		stubBackend: stubBackend{readStatus: func() (contractv1.StatusSnapshot, error) {
			current := snapshot
			if submitted.RequestID != "" {
				current.Operations = append(current.Operations, contractv1.Operation{
