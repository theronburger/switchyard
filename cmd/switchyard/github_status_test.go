...
func TestPullRequestCadenceStepsDownWithActivity(t *testing.T) {
	...
	observation := &contractv2.PullRequestObservation{Status: "found", PullRequest: &pullRequest}
...
func TestGitHubObserverPreservesLastKnownStateOnFailure(t *testing.T) {
	...
	snapshot.Repositories[0].Worktrees[0].PullRequest = &contractv2.PullRequestObservation{
...
func githubObserverSnapshot() contractv2.StatusSnapshot {
...
func testPullRequest(updatedAt time.Time) contractv2.PullRequest {
	...
	return contractv2.PullRequest{
		...
		Checks: contractv2.PullRequestChecks{
			...
			Items: []contractv2.PullRequestCheck{{
