...
`sy prepare <worktree-id|.> --wait` exposes adapter-owned workspace preparation without starting an environment. For Marketplace this resolves or provisions the `.nvmrc` Node toolchain, invokes the checked-in Yarn release with `install --immutable`, verifies the dependency artifacts, and publishes the resulting fingerprint and toolchain state. This is the setup command for Codex-created worktrees; repeated execution is cached only while both the source fingerprint and verifier remain valid. The complete wait is bounded to five minutes, tolerates transient daemon discovery gaps, and automatically resubmits an implicitly keyed preparation interrupted by a topology restart.
