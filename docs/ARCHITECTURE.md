...
Agent task lifecycle remains owned by the agent host. Codex creates each task and its worktree; the setup hook can call `sy prepare . --wait`, after which Switchyard discovers the external worktree. The app's Codex-specific surface is read-only cross-navigation: an exact-`cwd` task lookup through the local app-server followed by a verified `codex://threads/{id}` open. The reverse direction is the generic `sy open .` command and `switchyard://worktrees/{id}` app route. Switchyard does not create tasks, infer task occupancy, inspect transcripts, or inject prompts.
