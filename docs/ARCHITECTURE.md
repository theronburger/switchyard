...
- event history keeps the most recent 10,000 audit events (`operation.created`, `operation.transitioned`, `configuration.accepted`, `occupancy.acquired`, `occupancy.released`), each appended in the transaction that made the change;
